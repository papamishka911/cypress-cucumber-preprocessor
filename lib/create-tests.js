...
let specId = 0;
const internalSpecProperties = new Map();
function createInternalSpecProperties(properties) {
    internalSpecProperties.set(++specId, properties);
    return specId;
}
function retrieveInternalSpecProperties() {
    const reference = Cypress.env(constants_1.INTERNAL_SPEC_PROPERTIES);
    return (0, assertions_1.assertAndReturn)(internalSpecProperties.get(reference), `Expected to find internal spec properties with reference = ${reference}`);
}
...
function flushMessages(messages) {
    if (messages.enabled) {
        cy.task(constants_1.TASK_APPEND_MESSAGES, messages.stack.splice(0, messages.stack.length), { log: false });
    }
}
...
function createPickle(context, scenario, pickle) {
    ...
    const internalEnv = {
        [constants_1.INTERNAL_SPEC_PROPERTIES]: createInternalSpecProperties(internalProperties),
    };
    ...
    afterEach(function () {
        ...
        if (remainingSteps.length > 0) {
            if (((_a = this.currentTest) === null || _a === void 0 ? void 0 : _a.state) === "failed") {
                ...
                if (this.currentTest === stepAfter && ((_m = this.currentTest) === null || _m === void 0 ? void 0 : _m.state) === "failed") {
                ...
                for (const skippedStep of remainingSteps) {
                    const testStepId = (0, assertions_1.assertAndReturn)((_p = (_o = skippedStep.hook) === null || _o === void 0 ? void 0 : _o.id) !== null && _p !== void 0 ? _p : (_q = skippedStep.pickleStep) === null || _q === void 0 ? void 0 : _q.id, "Expected a step to either be a hook or a pickleStep");
                    messages.push({
                        testStepStarted: {
                            testStepId,
                            testCaseStartedId,
        ...
        const currentRetry = (0, assertions_1.assertAndReturn)((_r = this.currentTest) === null || _r === void 0 ? void 0 : _r._currentRetry, "Expected to find an attribute _currentRetry");
        const retries = (0, assertions_1.assertAndReturn)((_s = this.currentTest) === null || _s === void 0 ? void 0 : _s._retries, "Expected to find an attribute _retries");
        const willBeRetried = ((_t = this.currentTest) === null || _t === void 0 ? void 0 : _t.state) === "failed" ? currentRetry < retries : false;
        messages.push({
            testCaseFinished: {
                testCaseStartedId,
                timestamp: endTimestamp,
